...
def InserirAbastecimentos(lista_abastecimentos):
    ...
    if supabase:
        for abastecimento in lista_abastecimentos:
            ...
            result = supabase.table("abastecimentos").select("id").eq("documento", abastecimento[0]).eq("data", abastecimento[1]).eq("placa_veiculo", abastecimento[2]).eq("quantidade", float(abastecimento[6].replace(',', '.'))).execute().data
